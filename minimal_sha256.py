"""Minimal SHA-256 implementation to debug."""
...
# Test
tests = [b"", b"abc", b"a"*55, b"a"*56, b"a"*64]
for msg in tests:
    ref = hashlib.sha256(msg).hexdigest()
    mine = sha256(msg)
    match = "OK" if ref == mine else "MISMATCH"
    print(f"len={len(msg):2d}: ref={ref[:16]}... mine={mine[:16]}... {match}")
